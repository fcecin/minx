...
- `uint8_t VERSION`: Implementation identifier.
- `uint64_t PASSWORD`: A non-zero random value that is mapped to the receiver's IP address for an implementation-defined short time (e.g. 5 seconds), or zero if no password allocated for whatever reason.
...
- `uint8_t VERSION`: Implementation identifier.
- `uint64_t PASSWORD`: The `PASSWORD` value received in the most recent `INIT_ACK` response that the sender has received previously, or zero if none.
